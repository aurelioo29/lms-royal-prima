...
class QuizAttemptController extends Controller
{
    ...
    public function result(Course $course, CourseModule $module, QuizAttempt $attempt)
    {
        ...
        $hasEssayQuestion = $quiz->questions
            ->contains(fn($q) => $q->type === 'essay');

        $canShowResult =
            ! $hasEssayQuestion
            || in_array($attempt->status, ['reviewed_passed', 'reviewed_failed']);
    ...
    public function allowRetake(QuizAttempt $attempt)
    {
        ...
        $this->quizService->resetForRetake($attempt);

        return back()->with(
            'success',
            'Quiz berhasil di-reset. Peserta dapat mengerjakan ulang.'
        );
    }
}
