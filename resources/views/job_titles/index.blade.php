<x-app-layout>
    <div class="max-w-full">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-xl font-semibold text-slate-800">Job Titles</h1>
                <p class="text-sm text-slate-500">Jabatan/Profesi (Perawat, Dokter, HR, dll).</p>
            </div>

            <a href="{{ route('job-titles.create') }}"
                    ...
                    <tbody class="divide-y">
                        @forelse($titles as $t)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 text-slate-600">{{ $t->jobCategory?->name }}</td>
                                ...
                                <td class="px-4 py-3 text-right whitespace-nowrap">
                                    <a href="{{ route('job-titles.edit', $t) }}"
                                        class="text-[#121293] hover:underline mr-3">Edit</a>
                                    <form action="{{ route('job-titles.destroy', $t) }}" method="POST" class="inline"
                                        onsubmit="return confirm('Hapus job title ini?')">
                                        @csrf
                                        @method('DELETE')
