                        ...
                        <div class="min-w-0">
                            ...
                            <div class="mt-2 flex flex-wrap items-center gap-2">
                                ...
                                <span class="text-xs text-slate-500">
                                    Direktur dapat mengubah kapan saja. Kabid hanya dapat mengubah status
                                    Draft/Rejected.
                                </span>
                            </div>
                        </div>
                ...
                <form method="POST" action="{{ route('annual-plans.update', $annualPlan) }}"
                    ...
                    <div>
                        ...
                        <input id="title" name="title" type="text"
                            value="{{ old('title', $annualPlan->title) }}"
                            class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#121293]/20">
                        @error('title')
                            <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="5"
                            class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#121293]/20">{{ old('description', $annualPlan->description) }}</textarea>
                        @error('description')
                            <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    </div>
